...
df = pd.read_csv("wine_quality_red (5).csv")
...
st.write("2) The Mean Absolute Error of model is:", np.round(mt.mean_absolute_error(predictions, y_train ),2))

st.write("3) MSE: ", np.round(mt.mean_squared_error(predictions,y_train),2))

st.write("4) The R-Square score of the model is " , np.round(mt.r2_score(predictions,y_train ),2))
